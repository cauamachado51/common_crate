...
impl fmt::Display for PathTree<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		write!(f, "{}/", self.root.file_name().ok_or(fmt::Error)?.display())?;
...
fn write_tree(f: &mut fmt::Formatter<'_>, root: &Path, current_path: &Path, prefix: &str, include: &[&str], exclude: &[&str]) -> fmt::Result {
	...
	let exclude_n: Vec<String> = exclude.iter().map(|p| p.replace('\\', "/").trim_end_matches('/').to_owned()).collect();
	let include_n: Vec<String> = include.iter().map(|p| p.replace('\\', "/").trim_end_matches('/').to_owned()).collect();
...
fn excludes_of_gitignore<'a>(root: &'a Path, buf: &'a mut String) -> Vec<&'a str> {
