...
pub trait AsDisplay {
	...
	/// ### Exemplo
	/// ```
	/// use common_crate::fmt::AsDisplay;
	/// let vec = vec![1, 2, 3];
	/// let texto = "texto";
	/// println!("{}", vec.debug_to_display()); // [1, 2, 3]
	/// println!("{}", texto.debug_to_display()); // "texto"
	/// println!("{:?} {}", vec, texto); // [1, 2, 3] texto
	/// ```
    fn debug_to_display(&self) -> DebugToDisplay<'_, Self> where Self: Sized;
}

impl<T: Debug> AsDisplay for T {
    fn debug_to_display(&self) -> DebugToDisplay<'_, Self> {
        DebugToDisplay(self)
    }
}